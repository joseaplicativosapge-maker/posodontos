...
import {
  ...
  User, Calendar, DollarSign, Filter, FileText, Save
} from 'lucide-react';
import {
  Customer, Product,
...
//   name:         string;   // ej: "ORTODONCIA COMPLETA"
//   description?: string;   // descripción clínica opcional
// ─────────────────────────────────────────────────────────────────────────────
...
interface TreatmentViewProps {
  ...
  onAddTreatment:    (t: PatientTreatment) => void;
  onUpdateTreatment: (t: PatientTreatment) => void;
...
export const TreatmentView: React.FC<TreatmentViewProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.customerId)    { notify('Selecciona un paciente', 'warning'); return; }
    ...
    if (editingId) {
      onUpdateTreatment({ ...form, id: editingId, createdAt: now, updatedAt: now });
      notify('Tratamiento actualizado', 'success');
    } else {
      onAddTreatment({ ...form, id: `trx-${Date.now()}`, createdAt: now, updatedAt: now });
        ...
        <div className="space-y-3">
          {displayed.map(t => {
            ...
            return (
              <div
                key={t.id}
                className={`bg-white rounded-[2rem] border shadow-sm overflow-hidden transition-all ${cfg.border}`}
              >
                {/* Cabecera */}
                <div className="p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className={`${cfg.bg} ${cfg.text} text-[8px] font-black uppercase px-2 py-0.5 rounded-full`}>
                        {cfg.label}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => openEdit(t)}
                      className="p-2.5 bg-slate-50 text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-xl transition-all"
                      title="Editar"
                    >
                      <Edit2 size={15} />
                    </button>
                    ...
                    <button
                      ...
                      className="p-2.5 bg-slate-50 text-slate-400 hover:text-slate-700 rounded-xl transition-all"
                    >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {(ses.date || ses.time) && (
                                  <p className="text-[9px] text-slate-400 font-medium mt-0.5 flex items-center gap-1">
                                    <Calendar size={8} />
                                    {ses.date} {ses.time && `· ${ses.time}`}
                                  </p>
                                )}
                                {ses.notes && (
                                  <p className="text-[9px] text-slate-400 italic mt-1">{ses.notes}</p>
                                )}
                              </div>
                            </div>
                          );
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-200 flex flex-col max-h-[92vh]">
            ...
            <div className="p-7 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
              ...
              <button
                ...
                className="bg-white p-2 rounded-xl shadow-sm text-slate-400 hover:text-slate-700 transition-all"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-7 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-3">
                ...
                {selectedCustomer ? (
                  <div className="flex items-center justify-between bg-teal-50 border border-teal-100 rounded-2xl p-4">
                    ...
                    <button
                      type="button"
                      onClick={() => { setField('customerId', ''); setCustSearch(''); }}
                      ...
                      <div className="absolute top-full left-0 w-full bg-white shadow-2xl rounded-2xl mt-1 border border-slate-100 overflow-hidden z-50">
                        {filteredCusts.map(c => (
                          <button
                            key={c.id}
                            type="button"
                            onClick={() => { setField('customerId', c.id); setCustSearch(c.name); setShowDrop(false); }}
                    ...
                    <div key={ses.id} className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            type="text"
                            className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-bold uppercase border-none outline-none"
                            value={ses.label}
                            onChange={e => updateSes(idx, 'label', e.target.value)}
                            placeholder="Ej: Limpieza inicial"
                          />
                        </div>
                        <div>
                          <label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Estado</label>
                          <select
                            className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-black uppercase border-none outline-none"
                            value={ses.status}
                            onChange={e => updateSes(idx, 'status', e.target.value as SessionStatus)}
                          >
                        ...
                        <div>
                          ...
                          <input
                            type="date"
                            className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-bold border-none outline-none"
                            value={ses.date || ''}
                            onChange={e => updateSes(idx, 'date', e.target.value)}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-bold border-none outline-none"
                            value={ses.time || ''}
                            onChange={e => updateSes(idx, 'time', e.target.value)}
                          />
                        </div>
                      </div>
                      <div>
                        ...
                        <input
                          type="text"
                          className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-medium border-none outline-none"
                          value={ses.notes || ''}
                          onChange={e => updateSes(idx, 'notes', e.target.value)}
              ...
              <button
                type="submit"
                className="w-full bg-slate-900 text-white font-black py-5 rounded-[2rem] shadow-xl uppercase tracking-widest text-xs active:scale-95 transition-all flex items-center justify-center gap-2"
              >
                <Save size={16} />
                {editingId ? 'Guardar Cambios' : 'Asignar Tratamiento'}
              </button>
            </form>
          </div>
        </div>
