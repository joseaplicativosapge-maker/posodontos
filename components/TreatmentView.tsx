...
const blank = (branchId: string): FormData => ({
  customerId: '', productId: '', name: '', doctor: '',
  status: TreatmentStatus.PENDIENTE, sessions: [newSes(1)],
  totalCost: 0, notes: '', branchId,
});

const formatCOP = (v: number) =>
  new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 }).format(v);

// ─── ScheduleModal (sin cambios respecto al original) ─────────────────────────
...
const ScheduleModal: React.FC<ScheduleModalProps> = ({
  ...
  const getTomorrow = () => { const d = new Date(); d.setDate(d.getDate()); return d.toISOString().split('T')[0]; };
  ...
  const handleSave = async () => {
    ...
    if (isRescheduling && date === session.date && time === session.time) {
      notify('Debes seleccionar una fecha u hora diferente para reagendar', 'warning'); return;
    }
    setSaving(true);
    try {
      await dataService.saveReservation({
        id: `res-${Date.now()}-new`, branchId,
        customerName: customer?.name || 'Paciente', customerPhone: customer?.phone || '',
        customerEmail: customer?.email || '', date, time, seats: 1, status: 'PENDIENTE',
      ...
      onClose();
    } catch { notify('Error al agendar la cita', 'error'); }
          ...
          {isRescheduling && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-amber-700 mt-0.5">{session.date} · {session.time}</p>
                <p className="text-[9px] text-amber-500 font-medium mt-0.5">Selecciona una nueva fecha y hora para reagendar</p>
              </div>
            </div>
          )}
          {customer && (
            <div className="bg-teal-50 border border-teal-100 rounded-2xl p-4 flex items-center gap-3">
              <div className="bg-teal-600 p-2 rounded-xl text-white"><User size={14} /></div>
              <div>
                <p className="text-sm font-black text-teal-700 uppercase">{customer.name}</p>
                <p className="text-[9px] text-teal-400 font-bold mt-0.5">{customer.phone}{customer.documentNumber && ` · ${customer.documentNumber}`}</p>
              </div>
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[9px] font-black text-slate-400 uppercase mb-1.5 tracking-widest flex items-center gap-1"><Calendar size={10} />{isRescheduling ? 'Nueva Fecha *' : 'Fecha *'}</label>
              <input type="date" min={getTomorrow()} className="w-full bg-slate-50 rounded-2xl p-3 text-sm font-bold border-none outline-none focus:ring-2 focus:ring-teal-500" value={date} onChange={e => setDate(e.target.value)} />
            </div>
            <div>
              <label className="block text-[9px] font-black text-slate-400 uppercase mb-1.5 tracking-widest flex items-center gap-1"><Clock size={10} />{isRescheduling ? 'Nueva Hora *' : 'Hora *'}</label>
              <input type="time" className="w-full bg-slate-50 rounded-2xl p-3 text-sm font-bold border-none outline-none focus:ring-2 focus:ring-teal-500" value={time} onChange={e => setTime(e.target.value)} />
            </div>
          </div>
          <div className="bg-slate-50 rounded-2xl p-3 flex items-center gap-2">
            <Stethoscope size={14} className="text-slate-400 shrink-0" />
            <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Profesional</p><p className="text-sm font-bold text-slate-700">{treatment.doctor}</p></div>
          </div>
          <div>
            <label className="block text-[9px] font-black text-slate-400 uppercase mb-1.5 tracking-widest flex items-center gap-1"><FileText size={10} />{isRescheduling ? 'Motivo del reagendamiento' : 'Notas adicionales'}</label>
            <textarea rows={2} className="w-full bg-slate-50 border-none rounded-2xl p-3 font-medium text-sm outline-none focus:ring-2 focus:ring-teal-500 resize-none" value={notes} onChange={e => setNotes(e.target.value)} placeholder={isRescheduling ? 'Ej: Paciente solicitó cambio de horario...' : 'Indicaciones especiales...'} />
          </div>
          <div className="flex gap-4 pt-3">
            <button type="button" onClick={onClose} className="flex-1 h-12 rounded-2xl bg-white border border-slate-200 text-slate-600 font-bold text-xs uppercase tracking-widest transition-all duration-200 hover:bg-slate-100">Cancelar</button>
            <button type="button" onClick={handleSave} disabled={saving} className={`flex-1 h-12 rounded-2xl text-white font-bold text-xs uppercase tracking-widest shadow-lg transition-all duration-200 flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-60 ${isRescheduling ? 'bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700' : 'bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700'}`}>
...
export const TreatmentView: React.FC<TreatmentViewProps> = ({
  ...
  const [scheduleTarget, setScheduleTarget] = useState<{ treatment: PatientTreatment; session: TreatmentSession } | null>(null);
  ...
  const filteredCusts = useMemo(() => {
    ...
    return customers.filter(c => c.isActive && (
      c.name.toLowerCase().includes(q) || c.documentNumber?.includes(custSearch) || c.phone?.includes(custSearch)
    )).slice(0, 6);
  ...
  const openNew = () => {
    setEditingId(null); setForm(blank(currentBranchId)); setCustSearch('');
    setSelectedCustomerId(''); setShowDrop(false); setIsOpen(true);
  };

  const openEdit = (t: PatientTreatment) => {
    setEditingId(t.id);
    setForm({ customerId: t.customerId, productId: t.productId || '', name: t.name || '', doctor: t.doctor, status: t.status, sessions: t.sessions.map(s => ({...s})), totalCost: t.totalCost || 0, notes: t.notes || '', branchId: t.branchId });
    setSelectedCustomerId(t.customerId);
    setCustSearch(customers.find(c => c.id === t.customerId)?.name || '');
    setShowDrop(false); setIsOpen(true);
  };

  const selectCustomer = (c: Customer) => {
    setSelectedCustomerId(c.id); setForm(prev => ({...prev, customerId: c.id}));
    setCustSearch(c.name); setShowDrop(false);
  };

  const clearCustomer = () => {
    setSelectedCustomerId(''); setForm(prev => ({...prev, customerId: ''}));
    setCustSearch(''); setShowDrop(false);
  };

  const setField = (field: keyof FormData, val: any) => setForm(f => ({...f, [field]: val}));

  const handleProductSelect = (productId: string) => {
    const p = products.find(pr => pr.id === productId);
    setForm(f => ({ ...f, productId, name: p ? p.name : f.name, totalCost: p ? p.price : f.totalCost }));
  };
  ...
  const updateSes = (i: number, field: keyof TreatmentSession, val: any) => {
    const arr = [...form.sessions]; arr[i] = {...arr[i], [field]: val}; setField('sessions', arr);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { ...form, customerId: selectedCustomerId };
    setSaving(true);
    try {
      if (editingId) {
      ...
      setIsOpen(false);
    } catch { notify('Error al guardar el tratamiento', 'error'); }
    finally { setSaving(false); }
  };
  ...
  const handleScheduleSaved = async (sesId: string, date: string, time: string) => {
    ...
    const updatedSessions = treatment.sessions.map(s => {
      ...
      return { ...s, date, time, isScheduled: true, status: s.date && s.date !== date ? SessionStatus.REPROGRAMADA : SessionStatus.PROGRAMADA };
    });
    await onUpdateTreatment({ ...treatment, sessions: updatedSessions, createdAt: treatment.createdAt, updatedAt: new Date().toISOString() });
  };
    ...
    <div className="p-4 md:p-8 h-full overflow-y-auto bg-slate-50 pb-24">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:justify-between md:items-start gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium text-xs tracking-widest mt-1">Gestión de tratamientos asignados a pacientes</p>
        </div>
        <button onClick={openNew} className="bg-slate-900 text-white px-8 py-3.5 rounded-[2rem] font-black uppercase text-[10px] tracking-widest flex items-center gap-2 shadow-xl hover:bg-teal-600 transition-all active:scale-95 shrink-0">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por paciente, tratamiento o doctor..." className="w-full pl-11 pr-4 py-3.5 bg-white border-none rounded-2xl shadow-sm focus:ring-2 focus:ring-teal-500 font-medium text-sm outline-none" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <div className="relative">
          <Filter className="absolute left-3 top-3.5 text-slate-400" size={16} />
          <select className="pl-9 pr-6 py-3.5 bg-white border-none rounded-2xl shadow-sm font-black text-[10px] uppercase tracking-widest outline-none appearance-none cursor-pointer" value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)}>
      ...
      {displayed.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[45vh] text-center opacity-40">
          ...
          <p className="text-slate-400 text-sm mt-2 font-medium">
            {search || filterStatus !== 'TODOS' ? 'Sin resultados para los filtros aplicados' : 'Crea el primer tratamiento con el botón de arriba'}
          </p>
        </div>
      ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
                      ...
                      <span
                        className="text-[10px] text-slate-500 font-bold flex items-center gap-1
                          hover:text-teal-600 transition-colors cursor-pointer group/pname"
                        onClick={() => cust && setCustomerModal(cust)}
                        title="Ver ficha del paciente"
                      >
                        <User size={10} />
                        {cust?.name || 'Paciente no encontrado'}
                        {cust && <ChevronRight size={8} className="opacity-0 group-hover/pname:opacity-100 transition-opacity" />}
                      </span>
                    ...
                    {nextSes && (
                      <div className="flex items-center gap-1.5 mt-2">
                        ...
                        <span className="text-[9px] font-bold text-teal-600">Próxima: {nextSes.date} {nextSes.time && `· ${nextSes.time}`}</span>
                      </div>
                    )}
                    <div className="mt-3 flex items-center gap-3">
                      ...
                      <span className="text-[9px] font-black text-slate-400 shrink-0 tabular-nums">{done}/{t.sessions.length} · {prog}%</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => openEdit(t)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-xl transition-all" title="Editar"><Edit2 size={15} /></button>
                    <button
                      onClick={async () => {
                        const ok = await confirm({ title: '¿Eliminar tratamiento?', message: `Se eliminará "${t.name || 'este tratamiento'}" y todas sus sesiones.`, type: 'danger', confirmText: 'Sí, eliminar', cancelText: 'Cancelar' });
                        if (ok) await onDeleteTreatment(t.id);
                      }}
                      className="p-2.5 bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Eliminar"
                    >
                      <Trash2 size={15} />
                    </button>
                  ...
                  <div className="border-t border-slate-100 px-5 py-4 bg-slate-50/60 space-y-2 animate-in slide-in-from-top-2">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {pps && (
                        <span className="flex items-center gap-1 bg-teal-50 border border-teal-100 text-teal-600 text-[9px] font-black px-2.5 py-1 rounded-full">
                          <DollarSign size={9} />{formatCOP(pps)} / sesión
                        </span>
                      )}
                    </div>
                      ...
                      <div className="space-y-2">
                        {t.sessions.map((ses, idx) => {
                          ...
                          if (ses.date && ses.status !== SessionStatus.REALIZADA && ses.status !== SessionStatus.CANCELADA && ses.status !== SessionStatus.PAGADA) {
                            const sessionDate = new Date(ses.date); sessionDate.setHours(0, 0, 0, 0);
                          ...
                          return (
                            <div key={ses.id} className="flex items-start gap-3 bg-white rounded-2xl p-3.5 border border-slate-100">
                              ...
                              {canSchedule && (
                                <button type="button" onClick={() => setScheduleTarget({ treatment: t, session: ses })}
                                  className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest transition-all border ${hasDate ? 'bg-amber-50 text-amber-600 hover:bg-amber-500 hover:text-white border-amber-200' : 'bg-teal-50 text-teal-600 hover:bg-teal-600 hover:text-white border-teal-200'}`}
                                  title={hasDate ? 'Reagendar esta sesión' : 'Agendar cita para esta sesión'}
                                >
                                  <CalendarPlus size={12} />{hasDate ? 'Reagendar' : 'Agendar'}
                                </button>
                              )}
                            </div>
                          );
                    ...
                    {t.notes && (
                      <div className="mt-3 bg-teal-50 border border-teal-100 rounded-2xl p-3">
                        <p className="text-[9px] font-black text-teal-600 uppercase mb-1 flex items-center gap-1"><FileText size={9} /> Notas clínicas</p>
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[300] p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-200 flex flex-col max-h-[92vh]">
            <div className="p-7 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
              <div className="flex items-center gap-3">
                <div className="bg-teal-600 p-2 rounded-xl text-white shadow-lg"><Stethoscope size={20} /></div>
                <div>
                  <h3 className="text-xl font-black uppercase tracking-tighter text-slate-800">{editingId ? 'Editar Tratamiento' : 'Asignar Tratamiento'}</h3>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{editingId ? 'Modifica datos y sesiones' : 'Asigna un tratamiento a un paciente'}</p>
                </div>
              </div>
              <button onClick={() => setIsOpen(false)} className="bg-white p-2 rounded-xl shadow-sm text-slate-400 hover:text-slate-700 transition-all"><X size={20} /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-7 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-3">
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><User size={13} /> Paciente *</h4>
                {selectedCustomer ? (
                  <div className="flex items-center justify-between bg-teal-50 border border-teal-100 rounded-2xl p-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[9px] text-teal-400 font-bold mt-0.5">{selectedCustomer.phone}{selectedCustomer.documentNumber && ` · ${selectedCustomer.documentNumber}`}</p>
                    </div>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {/* ── Botón ver ficha dentro del modal de tratamiento ── */}
                      <button
                        type="button"
                        onClick={() => setCustomerModal(selectedCustomer)}
                        className="text-teal-400 hover:text-teal-600 p-1.5 rounded-xl hover:bg-teal-100 transition-colors"
                        title="Ver ficha completa del paciente"
                      >
                        <ChevronRight size={16} />
                      </button>
                      <button type="button" onClick={clearCustomer} className="text-red-400 hover:text-red-600 p-1.5 rounded-xl hover:bg-red-50 transition-colors"><X size={16} /></button>
                    </div>
                  </div>
                ) : (
                  <div className="relative">
                    <Search className="absolute left-3 top-3.5 text-slate-400" size={16} />
                    <input type="text" autoComplete="off" placeholder="Buscar por nombre, documento o teléfono..." className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-medium outline-none focus:ring-2 focus:ring-teal-500" value={custSearch} onChange={e => { setCustSearch(e.target.value); setShowDrop(true); }} onFocus={() => setShowDrop(true)} onBlur={() => setTimeout(() => setShowDrop(false), 200)} />
                    {showDrop && filteredCusts.length > 0 && (
                      <div className="absolute top-full left-0 w-full bg-white shadow-2xl rounded-2xl mt-1 border border-slate-100 overflow-hidden z-50">
                        {filteredCusts.map(c => (
                          <button key={c.id} type="button" onMouseDown={e => e.preventDefault()} onClick={() => selectCustomer(c)} className="w-full p-3.5 text-left hover:bg-teal-50 border-b border-slate-50 last:border-0 transition-all">
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Calendar size={13} /> Sesiones ({form.sessions.length})</h4>
                    {form.totalCost > 0 && form.sessions.length > 0 && (
                      <p className="text-[9px] font-black text-teal-600 mt-1 flex items-center gap-1">
                        <DollarSign size={9} />{formatCOP(form.totalCost / form.sessions.length)} por sesión
                      </p>
                    )}
                  </div>
                    ...
                    <div key={ses.id} className="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          {form.totalCost > 0 && form.sessions.length > 0 && (
                            <span className="text-[9px] font-black text-slate-400 flex items-center gap-0.5"><DollarSign size={8} />{formatCOP(form.totalCost / form.sessions.length)}</span>
                          )}
                        </div>
                        {form.sessions.length > 1 && (
                          <button type="button" onClick={() => removeSes(idx)} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                        )}
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div><label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Etiqueta</label><input type="text" className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-bold uppercase border-none outline-none" value={ses.label} onChange={e => updateSes(idx, 'label', e.target.value)} placeholder="Ej: Limpieza inicial" /></div>
                        <div><label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Estado</label><select className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-black uppercase border-none outline-none" value={ses.status} onChange={e => updateSes(idx, 'status', e.target.value as SessionStatus)}>{Object.values(SessionStatus).map(s => <option key={s} value={s}>{S_CFG[s].label}</option>)}</select></div>
                        <div><label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Fecha</label><input type="date" className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-bold border-none outline-none" value={ses.date || ''} onChange={e => updateSes(idx, 'date', e.target.value)} min={getTomorrow()} /></div>
                        <div><label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Hora</label><input type="time" className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-bold border-none outline-none" value={ses.time || ''} onChange={e => updateSes(idx, 'time', e.target.value)} /></div>
                      </div>
                      <div><label className="block text-[8px] font-black text-slate-400 uppercase mb-1">Notas de sesión</label><input type="text" className="w-full bg-slate-50 rounded-xl p-2.5 text-[11px] font-medium border-none outline-none" value={ses.notes || ''} onChange={e => updateSes(idx, 'notes', e.target.value)} placeholder="Observaciones..." /></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Notas clínicas */}
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest flex items-center gap-2"><FileText size={12} /> Notas Clínicas</label>
              ...
              <button type="submit" disabled={saving} className="w-full bg-slate-900 text-white font-black py-5 rounded-[2rem] shadow-xl uppercase tracking-widest text-xs active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-60">
                <Save size={16} />
